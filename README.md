...
| **Node.js 20.11 以上** | 公式インストーラー: <https://nodejs.org/ja/download>（LTS 版を選択）<br>※ このテンプレートが React/Vite を使うため必要です |
...
npm run msw:init
```
初回は数分かかります。エラーが出た場合は Node.js のバージョン (`node -v`) が 20.11 以上か確認してください。
...
```bash
# Node.js 20 をインストール（nvm 経由がおすすめ）
curl -o- https://raw.githubusercontent.com/nvm-sh/nvm/v0.40.1/install.sh | bash
# 一度ターミナルを開き直してから:
nvm install 20
nvm use 20
node -v   # → v20.x.x が出ればOK
```
...
| `npm install` でエラー | `node -v` が `v20.11` 以上か確認。古ければ Node.js を更新 |
